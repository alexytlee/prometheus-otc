...
(function(w, d, u) {
  ...
  if (getQSParameterByName("lang", w.location.href))
    ...
    parseURL(w.location.href, false)
      .substring(0, 4)
      .replace(/\//g, "").length == 2
  )
    translationLang = parseURL(w.location.href, false)
      .substring(0, 4)
      .replace(/\//g, "");
  else if (
    parseURL(w.location.href, true).substring(
      0,
      parseURL(w.location.href, true).indexOf(".")
    ).length == 2
  )
    translationLang = parseURL(w.location.href, true).substring(
      0,
      parseURL(w.location.href, true).indexOf(".")
    );
  else if (
    parseURL(w.location.href, true).substring(
      parseURL(w.location.href, true).lastIndexOf(".") + 1,
      parseURL(w.location.href, true).length
    ).length == 2
  )
    translationLang = parseURL(w.location.href, true).substring(
      parseURL(w.location.href, true).lastIndexOf(".") + 1,
  ...
  if (translationLang) d.body.style.visibility = "hidden";
  ...
  TreeWalker.prototype.parentNode = function() {
    ...
    do {
      ...
      this._getFilteredStatus(testNode) !== this.NodeFilter.FILTER_ACCEPT
    );
  ...
  TreeWalker.prototype._getFilteredStatus = function(node) {
    var mask = {
      ...
      /* PROCESSING_INSTRUCTION_NODE */ 7: this.NodeFilter
    ...
    return mask && (this.whatToShow & mask) == 0
  ...
  function parseURL(url, domainOnly) {
    ...
    if (url.indexOf("://") > -1) {
      domain = url.split("/")[2];
      if (!domainOnly) domain = url.split("/")[0] + "//" + url.split("/")[2];
    } else domain = url.split("/")[0];

    if (domainOnly) return domain;
    else return url.replace(domain, "");
  }
  ...
  function isTranslatableSegment(html) {
    var foundPunctuation = html.match(
      /^(.|,|;|:|«|»|·|&|=|\/|\$|€|£|\(|\)|\*|\-|\+|\||\$-\/:-?{-~||\t|\r|\n|\d|\s)+$/gim
    );
    if (foundPunctuation) {
      ...
      if (!foundNonPunctuationChars) return false;
    }
  ...
  function rewriteUrl(link, absoluteLink, translationLang, url_pattern) {
    if (url_pattern == "?") {
      ...
      if (link.attributes["href"].value.indexOf("#") != -1) {
        ...
        link.attributes["href"].value = link.attributes["href"].value.replace(
          "#" + fragment,
          ""
        );
      }
      ...
      if (getQSParameterByName("lang", link.attributes["href"].value))
        link.attributes["href"].value = link.attributes["href"].value
          ...
          .replace(
            "lang=" +
              getQSParameterByName("lang", link.attributes["href"].value),
            "lang=" + translationLang
          );
      else if (link.attributes["href"].value.indexOf("?") != -1)
        link.attributes["href"].value += "&lang=" + translationLang;
      else link.attributes["href"].value += "?lang=" + translationLang;

      if (fragment) link.attributes["href"].value += "#" + fragment;
    } else if (url_pattern == "/") {
      if (
        parseURL(absoluteLink, false)
          .substring(0, 4)
          .replace(/\//g, "")
          .toLowerCase() == translationLang
      )
        link.attributes["href"].value =
          absoluteLink.split("/")[0] +
          "//" +
          parseURL(absoluteLink, true) +
          parseURL(absoluteLink, false);
      else
        link.attributes["href"].value =
          absoluteLink.split("/")[0] +
  ...
  function encodeAllSpecialTags(html) {
    html = html
      ...
      .replace(/<\/abbr>/gim, "&lt;/abbr&gt;")
      .replace(/<\/sub>/gim, "&lt;/sub&gt;")
      .replace(/<\/sup>/gim, "&lt;/sup&gt;")
      .replace(/<\/big>/gim, "&lt;/big&gt;")
      .replace(/<\/small>/gim, "&lt;/small&gt;");
    ...
    if (matches) {
      for (var i = 0; i < matches.length; i++) {
        html = html.replace(
          searchText,
          "&lt;" +
            matches[i].substring(1, matches[i].length - 1).toLowerCase() +
            "&gt;"
        );
      }
    }
  ...
  function translateDOM() {
    if (linguJSON) {
      ...
      if (languages.length > 1) {
        ...
        if (sourceLang.url_pattern) url_pattern = sourceLang.url_pattern;
        var menuHeight = 0;
        if (translationLang == undefined)
          translationLang = sourceLang.lang_code;
        var currlangcode =
          translationLang != "" ? translationLang : sourceLang.lang_code;
        var j = 1;
        for (var i = 0; i < languages.length; i++) {
          if (languages[i].lang_code != currlangcode) {
            ...
            if (!languages[i].url_pattern || languages[i].url_pattern == "?") {
              ...
              if (getQSParameterByName("lang", w.location.href))
                href = w.location.href
                  .toLowerCase()
                  .replace(
                    "lang=" + getQSParameterByName("lang", w.location.href),
                    "lang=" + languages[i].lang_code
                  );
              else if (w.location.href.indexOf("?") != -1)
                href = w.location.href + "&lang=" + languages[i].lang_code;
              else href = w.location.href + "?lang=" + languages[i].lang_code;
            } else if (languages[i].url_pattern == "/") {
              if (
                parseURL(w.location.href, false)
                  .substring(0, 4)
                  .replace(/\//g, "")
                  .toLowerCase().length == 2
              )
                href =
                  w.location.protocol +
                  "//" +
                  parseURL(w.location.href, true) +
                  parseURL(w.location.href, false).replace(
                    parseURL(w.location.href, false).substring(0, 4),
            ...
            if (
              ...
              customLangMenuHtml = customLangMenuHtml.replace(
                "[[linguTargetLang" + j + "]]",
                languages[i].lang_name
              );
              customLangMenuHtml = customLangMenuHtml.replace(
                "[[linguTargetLangHref" + j + "]]",
                href
              );
            } else {
        ...
        if (langMenu.addEventListener) {
          langMenu.addEventListener("mouseover", function(event) {
            d.getElementById("lingumania_langswitcher").style.height =
              menuHeight + "px";
          });
        ...
        var customMenuContainer = d.getElementById(
          linguJSON.custom_lang_switcher_container_id
        );
        if (!linguJSON.translated_pages) {
          ...
          for (var i = 0; i < linguJSON.translated_pages.length; i++) {
            var comparableLink = createComparableLink(
              w.location.href,
              languages
            );
            if (
              comparableLink.replace(currlangcode, "").replace("//", "/") ==
              linguJSON.translated_pages[i].slug.toLowerCase()
            ) {
              isTranslated = true;
              break;
            }
      ...
      if (
        linguJSON.translated_segments &&
        linguJSON.translated_segments.length > 0 &&
        isTranslated &&
        currlangcode != sourceLang.lang_code
      ) {
        var translatedSegments = linguJSON.translated_segments;

        var specialTags = getElementsByTagNames([
          "b",
          "u",
          "i",
          "strong",
        ...
        for (var i = 0; i < specialTags.length; i++) {
          if (specialTags[i].parentNode)
            specialTags[i].parentNode.innerHTML = encodeAllSpecialTags(
              specialTags[i].parentNode.innerHTML
            );
        }
        ...
        var node,
          ...
          fragments = [],
          linkTranslations = [];
        var domWalker = d.createTreeWalker(
          ...
          if (node.nodeValue != null) {
            ...
            if (canBeTranslated) {
              try {
                ...
                var startingWhiteSpaceMatches = startingWhiteSpaceRegex.exec(
                  node.nodeValue
                );
                var endingWhiteSpaceRegex = /\s+$/gim;
                var endingWhiteSpaceMatches = endingWhiteSpaceRegex.exec(
                  node.nodeValue
                );
                ...
                for (var i = 0; i < translatedSegments.length; i++) {
                  if (translatedSegments[i].target == undefined) {
                    if (eval("translatedSegments[i].target_" + currlangcode))
                      translatedSegments[i].target = eval(
                        "translatedSegments[i].target_" + currlangcode
                      );
                  }

                  if (
                    translatedSegments[i].source == node.nodeValue.trim() &&
                    translatedSegments[i].target
                  ) {
            ...
            if (
              node.nodeValue.match(
                /(<\/b|<b\s[^>]*>|<\/u>|<u\s[^>]*>|<\/i>|<i\s[^>]*>|<\/strong>|<strong[^>]*>|<\/em>|<em\s[^>]*>|<\/abbr>|<abbr[^>]*>|<\/sub>|<sub[^>]*>|<\/sup>|<sup[^>]*>|<\/big>|<big[^>]*>|<\/small>|<small[^>]*>)/gim
              )
            ) {
              var wrap = d.createElement("span");
              var frag = d.createDocumentFragment();
        ...
        for (var i = 0; i < imgs.length; i++) {
          ...
          if (
            img.attributes["alt"] &&
            isTranslatableSegment(img.attributes["alt"].value.trim())
          ) {
            for (var j = 0; j < translatedSegments.length; j++) {
              if (
                translatedSegments[j].source ==
                img.attributes["alt"].value.trim()
              ) {
        ...
        for (var i = 0; i < metas.length; i++) {
          ...
          if (meta.attributes["content"]) {
            for (var j = 0; j < translatedSegments.length; j++) {
              if (
                translatedSegments[j].source ==
                meta.attributes["content"].value.trim()
              ) {
        ...
        for (var i = 0; i < translatedSegments.length; i++) {
          ...
          if (
            translatedSegments[i].target &&
            translatedSegments[i].target.startsWith("http")
          )
        ...
        for (var i = 0; i < links.length; i++) {
          ...
          if (
            ...
            link.parentNode.className != "notranslate"
          ) {
            for (var j = 0; j < linkTranslations.length; j++) {
              if (
                linkTranslations[j].source.trim() ==
                link.attributes["href"].value.trim().replace("/../", "/")
              ) {
            ...
            if (
              link.attributes["href"].value.indexOf(
                parseURL(w.location.href, true)
              ) != -1 ||
              link.attributes["href"].value.indexOf(sourceLang.url_pattern) !=
                -1 ||
              !link.attributes["href"].value.toLowerCase().startsWith("http")
            ) {
              ...
              for (var j = 0; j < linkTranslations.length; j++) {
                var comparableLink = createComparableLink(
                  absoluteLink.trim().replace("/../", "/"),
                  languages
                );
                if (
                  parseURL(linkTranslations[j].source.trim(), false) ==
                  comparableLink
                ) {
                  link.attributes["href"].value = linkTranslations[j].target;
                  break;
              ...
              if (!linguJSON.translated_pages) {
                ...
                for (var j = 0; j < linguJSON.translated_pages.length; j++) {
                  if (
                    comparableLink ==
                    linguJSON.translated_pages[j].slug.toLowerCase()
                  ) {
                    rewriteUrl(
                      link,
                      absoluteLink,
      ...
      if (
        linguJSON.translated_image_segments &&
        linguJSON.translated_image_segments.length > 0 &&
        isTranslated &&
        currlangcode != sourceLang.lang_code
      ) {
          ...
          if (img.attributes["src"]) {
            for (var j = 0; j < translatedImageSegments.length; j++) {
              if (translatedImageSegments[j].img_target == undefined) {
                if (
                  eval("translatedImageSegments[j].img_target_" + currlangcode)
                )
                  translatedImageSegments[j].img_target = eval(
                    "translatedImageSegments[j].img_target_" + currlangcode
                  );
              }
              if (
                translatedImageSegments[j].img_source
                  .replace("http://", "")
                  .replace("https://", "")
                  .endsWith(
                ...
                img.attributes["src"].value =
                  translatedImageSegments[j].img_target;
                break;
              }
            }
          }
  ...
  function createComparableLink(absoluteLink, languages) {
    var comparableLink = absoluteLink
      .replace("http://", "")
      .replace("https://", "")
      .toLowerCase();

    if (
      comparableLink == parseURL(w.location.href, true) ||
      comparableLink == languages[0].url_pattern
    )
      comparableLink = "/";

    comparableLink = comparableLink.replace(
      parseURL(w.location.href, true),
      ""
    );
    ...
    if (!comparableLink.startsWith("/")) comparableLink = "/" + comparableLink;
  ...
  var linguLoader = function() {
    var cssCode =
      "a.lingumania_target, a.lingumania_poweredby { display: none; background-color: #999; } #lingumania_custom_id { display: block; color: #fff; display: block; width: 48px; padding: 6px; text-decoration: none; } a.lingumania_target:hover, a.lingumania_poweredby:hover { background-color: #000; } #lingumania_langswitcher{ position: absolute; top: 0px; right: 0px; z-index: 100001; text-transform: uppercase; text-align: left; color: #fff; font-size: 12px; line-height: 18px; } #lingumania_langswitcher:hover a { display: block; position: relative; z-index: 100002; float: right; width: 48px; padding: 6px; clear: both; color: #fff; text-decoration: none; } a#lingumania_currentlanglink { display: block; width: 48px; padding: 6px; background: url('//az596610.vo.msecnd.net/arrow-down-black.png') right top no-repeat; color: #fff; font-weight: bold; text-decoration: none; }  .lingumania_poweredby { background: right center no-repeat; border-top: 1px solid #808080; font-size: 9px; line-height: 12px; }";
    var style = d.createElement("style");
    style.type = "text/css";
    if (style.styleSheet) style.styleSheet.cssText = cssCode;
    else style.innerHTML = cssCode;
  ...
  w.addEventListener
    ? w.addEventListener("load", linguLoader, false)
    : w.attachEvent("onload", linguLoader);
})(window, document);
